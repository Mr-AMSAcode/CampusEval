<x-app-layout>
    <x-slot name="header">
        <div class="flex items-center gap-3">
            ...
            <span class="inline-flex items-center rounded-full bg-violet-100 px-3 py-1 text-xs font-semibold text-violet-900">
                {{ __('Your Statistics') }}
            </span>
        </div>
    </x-slot>
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-6">
            ...
            <div class="grid grid-cols-1 xl:grid-cols-3 gap-4">
                <div class="xl:col-span-2 bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl shadow-sm p-5">
                    <h3 class="font-semibold text-gray-900 dark:text-white">{{ __('Recent feedbacks (anonymous)') }}</h3>
                ...
                <div class="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-2xl shadow-sm p-5">
                    <h3 class="font-semibold text-gray-900 dark:text-white">{{ __('Classes you teach') }}</h3>
                    <div class="mt-4 space-y-3">
                        @foreach($classes as $c)
                            <div class="p-3 rounded-xl bg-slate-50 dark:bg-slate-800 border border-slate-100 dark:border-slate-700">
                                ...
                                <div class="text-xs text-slate-500">{{ $c->code }} · {{ $c->level }}</div>
                            </div>
                        @endforeach
                        @if($classes->isEmpty())
                            <div class="text-sm text-slate-600 dark:text-slate-300">{{ __('No classes assigned.') }}</div>
                        @endif
                    </div>
                </div>
            </div>
